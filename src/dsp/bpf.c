...
eq_update(rbj_eq *eq, float sample) {
	...
	/* Finally, compute y[0] */
	/* Note: b1 is zero for bpf, which is the only filter type we're using.
...
void
eq_create_bpf(rbj_eq *eq, double freq, double bandwidth) {
	double omega0 = 2 * 3.1415926535897932384 * freq;
	double sin_omega0 = sin(omega0);
	double alpha = sin_omega0 * sinh((log(2) / 2) * bandwidth * sin_omega0);
